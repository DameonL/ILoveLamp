class NewItemBinder {
    ...
    #bindFieldToElement(boundField, boundElement, fieldPath) {
        if (((boundElement.nodeName === "DIV") || (boundElement.nodeName === "SPAN")) && (boundField.stringValue)) {
            boundElement.innerText = boundField.stringValue;
        }
        else if ((boundElement.nodeName === "IMG") && (boundField.stringValue)) {
            boundElement.src = `../img/products/${this.#productId}/${boundField.stringValue}.jpg`;
        }
        else if (boundField.arrayValue) {
            ...
            for (let boundChildField of boundChildFields) {
                ...
                if (boundChildField.mapValue) {
                    let childBinder = new NewItemBinder(boundChildField.mapValue, boundChildElement, this.#productId, fieldPath);
                } else {
